...
class PlayerListBottomSheet : BottomSheetDialogFragment() {
    ...
    private fun showPlayers(players: List<Player>) {
        ...
        // Строки плееров появляются позже, чем сама шторка (после ответа
        // сети) — к этому моменту система уже расставила начальный фокус по
        // тому, что было на экране при открытии шторки (пустой список), и
        // сама на новые строки не переключится. Без этого пульт не отвечает
        // на первое нажатие OK: фокуса нет ни на чём, пока пользователь не
        // подвинет его стрелкой (задача 8.1). Переносим фокус на первую
        // строку сами, как только она появилась.
        playerList.getChildAt(0)?.requestFocus()
    }
