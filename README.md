...
Prism.Client (Vite/React)  ──HTTP/CORS──>  Prism.Host (ASP.NET Core / Kestrel)
                                            ...
                                            ├── GET /api/media/{id}      одна запись
                                            ├── GET /raw/{id}            прямой стрим с Range (браузерные файлы)
                                            ├── GET /hls/{id}/playlist.m3u8  HLS-плейлист (VOD)
                                            └── GET /hls/{id}/segment/N.ts   сегмент H264/H265 на лету
```
...
| `SessionMinutes`   | `15`         | длина одной сессии транскодирования (реже разрывы ↔ больше CPU/диска) |
